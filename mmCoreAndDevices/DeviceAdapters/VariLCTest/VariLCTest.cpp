///////////////////////////////////////////////////////////////////////////////
// FILE:          VariLC.cpp
// PROJECT:       Micro-Manager
// SUBSYSTEM:     DeviceAdapters
//-----------------------------------------------------------------------------
// DESCRIPTION:   VariLC 
//
// AUTHOR:        Rudolf Oldenbourg, MBL, w/ Arthur Edelstein and Karl Hoover, UCSF, Sept, Oct 2010
// COPYRIGHT:     
// LICENSE:  

// Change Log - Nick Anthony - Sept 11, 2018
// 1. Wavelength upper and lower limits are now queried from the device rather than being assumed to be 400-800 nm.
// 2. In the event that an out of range wavelength is set the adapter will now log an error and continue running. Previously this would result in a sudden crash without any log.

// Change Log - Amitabh Verma - Jan. 26, 2015
// 1. Error response retrieval using 'R?' fixed

// Change Log - Amitabh Verma - July. 23, 2014
// 1. Replaced ',' comma with ';' semi-colon  in property names due to Micro-Manager warning during HW Config Wizard 'Contains reserved chars'
// Note: This will break Pol-Acquisition (OpenPolScope) and requires compatible version which uses same name for Property

// Change Log - Amitabh Verma - Apr. 02, 2014
// 1. Variable time delay
// 2. Absolute Retardance in nm. property

// Change Log - Amitabh Verma, Grant Harris - Nov. 10, 2012
// 1. Implemented Number of Active LCs, Total No. of LCs and baud value for searching appropriate VariLC (9600) or Abrio (115200)

// Change Log - Amitabh Verma, Grant Harris - Oct 10, 2012
// 1. Implemented COM port Search ability using Micro-Manager Hardware Wizard

// Change Log - Amitabh Verma, Grant Harris - Aug 08, 2012
// 1. State commands 0-5 which implement Palette

// Change Log - Amitabh Verma, Grant Harris - Apr 27, 2012
// 1. Added Version numbering
// 2. Serial number device property
// 3. Total Number of LCs default value to "2" instead of "4"
// 4. wavelength default 546


#include "VariLCTest.h"
#include <cstdio>
#include <cctype>
#include <string>
#include <math.h>
#include "ModuleInterface.h"
#include <sstream>
#include <algorithm>    // std::remove_if
#include "Utils.h"



const char* g_ControllerName    = "VariLCTest";

const char* g_TxTerm            = "\r"; //unique termination
const char* g_RxTerm            = "\r"; //unique termination

const char* g_BaudRate_key        = "Baud Rate";
const char* g_Baud9600            = "9600";
const char* g_Baud115200          = "115200";



using namespace std;

///////////////////////////////////////////////////////////////////////////////
// Exported MMDevice API
///////////////////////////////////////////////////////////////////////////////
MODULE_API void InitializeModuleData()
{
	string log = "VariLCTest   InitializeModuleData";

	CUtils::cameraLog(log);

	RegisterDevice(g_ControllerName, MM::GenericDevice, "VariLCTest");
}

MODULE_API MM::Device* CreateDevice(const char* deviceName)
{

	string log = "VariLCTest  CreateDevice";

	CUtils::cameraLog(log);

	if (deviceName == 0) return 0;
	if (strcmp(deviceName, g_ControllerName) == 0) return new VariLCTest();
	return 0;
}

MODULE_API void DeleteDevice(MM::Device* pDevice)
{
	string log = "VariLCTest  DeleteDevice";

	CUtils::cameraLog(log);
	delete pDevice;
}

// General utility function:
int ClearPort(MM::Device& device, MM::Core& core, std::string port)
{
	string log = " VariLCTest  ClearPort  " + port;

	CUtils::cameraLog(log);

	// Clear contents of serial port 
	const int bufSize = 2048;
	unsigned char clear[bufSize];
	unsigned long read = bufSize;
	int ret;
	while ((int)read == bufSize)
	{
		ret = core.ReadFromSerial(&device, port.c_str(), clear, bufSize, read);
		if (ret != DEVICE_OK)
			return ret;
	}
	return DEVICE_OK;
}





VariLCTest::VariLCTest():
baud_(g_Baud9600),
initialized_(false),
initializedDelay_(false),
wavelength_(546),
numTotalLCs_(2),
numPalEls_(5),
numActiveLCs_(2)
{
	InitializeDefaultErrorMessages();
	
	string log = "VariLCTest   port_ == "  + port_;

	CUtils::cameraLog(log);

	// Port:
	CPropertyAction* pAct = new CPropertyAction(this, &VariLCTest::OnPort);
	CreateProperty(MM::g_Keyword_Port, "Undefined", MM::String, false, pAct, true);
	SetProperty(MM::g_Keyword_Port, port_.c_str());

	pAct = new CPropertyAction(this, &VariLCTest::OnNumActiveLCs);
	CreateProperty("Number of Active LCs", "2", MM::Integer, false, pAct, true);


	AddAllowedValue(g_BaudRate_key, g_Baud115200, (long)115200);
	AddAllowedValue(g_BaudRate_key, g_Baud9600, (long)9600);
		
}


VariLCTest::~VariLCTest()
{

	string log = "VariLCTest  ~VariLCTest";

	CUtils::cameraLog(log);

	Shutdown();

}

void VariLCTest::GetName(char* name) const
{
	string strCommand_down ;
    strCommand_down.assign(name,strlen(name) );


	string log = "VariLCTest  GetName    " + strCommand_down;

	CUtils::cameraLog(log);

	CDeviceUtils::CopyLimitedString(name, g_ControllerName);
}

bool VariLCTest::SupportsDeviceDetection(void)
{
	string log = "VariLCTest  SupportsDeviceDetection    " ;

	CUtils::cameraLog(log);
	return true;
}


MM::DeviceDetectionStatus VariLCTest::DetectDevice(void)
{
	// all conditions must be satisfied...
	MM::DeviceDetectionStatus result = MM::Misconfigured;

	string log = "VariLCTest  DetectDevice    " ;

	CUtils::cameraLog(log);

	try
	{
		long baud;
		GetProperty(g_BaudRate_key, baud);

		std::string transformed = port_;
		for (std::string::iterator its = transformed.begin(); its != transformed.end(); ++its)
		{
			*its = (char)tolower(*its);
		}

		if (0 < transformed.length() && 0 != transformed.compare("undefined") && 0 != transformed.compare("unknown"))
		{
			int ret = 0;
			MM::Device* pS;


			// the port property seems correct, so give it a try
			result = MM::CanNotCommunicate;
			// device specific default communication parameters
			GetCoreCallback()->SetDeviceProperty(port_.c_str(), MM::g_Keyword_AnswerTimeout, "2000.0");
			GetCoreCallback()->SetDeviceProperty(port_.c_str(), MM::g_Keyword_BaudRate, baud_.c_str());
			GetCoreCallback()->SetDeviceProperty(port_.c_str(), MM::g_Keyword_DelayBetweenCharsMs, "0.0");
			GetCoreCallback()->SetDeviceProperty(port_.c_str(), MM::g_Keyword_Handshaking, "Off");
			GetCoreCallback()->SetDeviceProperty(port_.c_str(), MM::g_Keyword_Parity, "None");
			GetCoreCallback()->SetDeviceProperty(port_.c_str(), MM::g_Keyword_StopBits, "1");
			GetCoreCallback()->SetDeviceProperty(port_.c_str(), "Verbose", "1");
			pS = GetCoreCallback()->GetDevice(this, port_.c_str());
			pS->Initialize();

			ClearPort(*this, *GetCoreCallback(), port_);
			ret = sendCmd("V?", serialnum_);
			if (ret != DEVICE_OK || serialnum_.length() < 5)
			{
				LogMessageCode(ret, true);
				LogMessage(std::string("VariLC not found on ") + port_.c_str(), true);
				LogMessage(std::string("VariLC serial no:") + serialnum_, true);
				ret = 1;
				serialnum_ = "0";
				pS->Shutdown();
			}
			else
			{
				// to succeed must reach here....
				LogMessage(std::string("VariLC found on ") + port_.c_str(), true);
				LogMessage(std::string("VariLC serial no:") + serialnum_, true);
				result = MM::CanCommunicate;
				GetCoreCallback()->SetSerialProperties(port_.c_str(),
					"600.0",
					baud_.c_str(),
					"0.0",
					"Off",
					"None",
					"1");
				serialnum_ = "0";
				pS->Initialize();
				ret = sendCmd("R1");
				pS->Shutdown();
			}
		}
	}
	catch (...)
	{
		LogMessage("Exception in DetectDevice!", false);
	}
	return result;
}



bool VariLCTest::Busy()
{
	string log = "VariLCTest  Busy";

	CUtils::cameraLog(log);

   if (delay.getMsec() > 0.0) {
      MM::MMTime interval = GetCurrentMMTime() - changedTime_;      
      if (interval.getMsec() < delay.getMsec() ) {
         return true;
      }
   }
   return false;
}

int VariLCTest::OnDelay(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	string log = "VariLCTest  OnDelay";

	CUtils::cameraLog(log);

   if (eAct == MM::BeforeGet)
   {
	 double delay = GetDelayMs();
	 initializedDelay_ = true;
	 pProp->Set(delay);
    }
   else if (eAct == MM::AfterSet)
   {
	  double delayT;
	  pProp->Get(delayT);
	  if (initializedDelay_) {
		SetDelayMs(delayT);
	  }
	  delay = delayT*1000;
   }
   return DEVICE_OK;
}

std::string VariLCTest::DoubleToString(double N)
{
    ostringstream ss("");
    ss << N;
    return ss.str();
}

int VariLCTest::Initialize()
{

	
	for (long i = 0; i < numTotalLCs_; ++i) {
		retardance_[i] = 0.5;
	}

	for (long i = 0; i < numPalEls_; i++) {
		palEl_[i] = "";
	}

	
		// Name
	int ret = CreateProperty(MM::g_Keyword_Name, g_ControllerName, MM::String, true);
	if (DEVICE_OK != ret)
		return ret;

	string log = "VariLCTest  Initialize  ret =  " + to_string(static_cast<long long>(ret)) + "   g_ControllerName  =   " + g_ControllerName;
	CUtils::cameraLog(log);
	// empty the Rx serial buffer before sending command
	
	ret = ClearPort(*this, *GetCoreCallback(), port_);
	if (ret != DEVICE_OK)
		return ret;
	

	
	// Version number
	CPropertyAction* pAct = new CPropertyAction(this, &VariLCTest::OnSerialNumber);
	ret = CreateProperty("Version Number", "Version Number Not Found", MM::String, true, pAct);
	if (ret != DEVICE_OK)
		return ret;

	
	// Active LC number
	pAct = new CPropertyAction(this, &VariLCTest::OnNumActiveLCs);
	ret = CreateProperty("Active LCs", "0", MM::Integer, true, pAct);
	if (ret != DEVICE_OK)
		return ret;

	// Total LC number
	pAct = new CPropertyAction(this, &VariLCTest::OnNumTotalLCs);
	ret = CreateProperty("Total LCs", "0", MM::Integer, true, pAct);
	if (ret != DEVICE_OK)
		return ret;

	pAct = new CPropertyAction(this, &VariLCTest::OnBriefMode);
	ret = CreateProperty("Mode; 1=Brief; 0=Standard", "", MM::String, true, pAct);
	if (ret != DEVICE_OK)
		return ret;

	//Set VariLC to Standard mode
	briefModeQ_ = false;
	ret = sendCmd("B0",getFromVariLC_);
	if (ret != DEVICE_OK)
		return ret;

	// Wavelength
	std::string ans;
	ret = sendCmd("V?", ans);	//The serial number response also contains the tuning range of the device
	std::vector<double> nums = getNumbersFromMessage(ans, briefModeQ_);	//This will be in the format (revision level, shortest wavelength, longest wavelength, serial number).
	if (ret != DEVICE_OK)
		return ret;
	pAct = new CPropertyAction(this, &VariLCTest::OnWavelength);
	ret = CreateProperty("Wavelength", DoubleToString(wavelength_).c_str(), MM::Float, false, pAct);
	if (ret != DEVICE_OK)
		return ret;
	SetPropertyLimits("Wavelength", nums.at(1), nums.at(2));

	// Delay
	pAct = new CPropertyAction(this, &VariLCTest::OnDelay);
	ret = CreateProperty("Device Delay (ms.)", "200.0", MM::Float, false, pAct);
	if (ret != DEVICE_OK)
		return ret;
	SetPropertyLimits("Device Delay (ms.)", 0.0, 200.0);

	CPropertyActionEx *pActX = 0;
	//	 create an extended (i.e. array) properties 0 through 1

	for (long i = 0; i < numActiveLCs_; ++i) {
		ostringstream s;
		s << "Retardance LC-" << char(65 + i);
		pActX = new CPropertyActionEx(this, &VariLCTest::OnRetardance, i);
		CreateProperty(s.str().c_str(), "0.5", MM::Float, false, pActX);
		SetPropertyLimits(s.str().c_str(), 0.0001, 3);
	}

	// Absolute Retardance controls -- after Voltage controls
	for (long i = 0; i < numActiveLCs_; ++i) {
		ostringstream s;
		s << "Retardance LC-" << char(65 + i) << " [in nm.]";
		pActX = new CPropertyActionEx(this, &VariLCTest::OnAbsRetardance, i);
		CreateProperty(s.str().c_str(), "100", MM::Float, true, pActX);
	}
	for (long i = 0; i < numPalEls_; ++i) {
		ostringstream s;
		std::string number;

		std::stringstream strstream;
		strstream << i;
		strstream >> number;
		if (i < 10) {
			number = "0" + number;
		}
		s << "Pal. elem. " << number << "; enter 0 to define; 1 to activate";
		pActX = new CPropertyActionEx(this, &VariLCTest::OnPalEl, i);
		CreateProperty(s.str().c_str(), "", MM::String, false, pActX);
	}
	pAct = new CPropertyAction(this, &VariLCTest::OnSendToVariLC);
	ret = CreateProperty("String send to VariLC", "", MM::String, false, pAct);
	if (ret != DEVICE_OK) {
		return ret;
	}
	pAct = new CPropertyAction(this, &VariLCTest::OnGetFromVariLC);
	ret = CreateProperty("String from VariLC", "", MM::String, true, pAct);
	if (ret != DEVICE_OK) {
		return ret;
	}
	SetErrorText(99, "Device set busy for ");
	return DEVICE_OK;
}

int VariLCTest::Shutdown() {
	
	string log = "VariLCTest  Shutdown";

	CUtils::cameraLog(log);

	initialized_ = false;
	return DEVICE_OK;
}

//////////////// Action Handlers (VariLCTest) /////////////////

int VariLCTest::OnPort(MM::PropertyBase* pProp, MM::ActionType pAct)
{
	string log = "VariLCTest  OnPort";

	CUtils::cameraLog(log);
	if (pAct == MM::BeforeGet)
	{
		string log = "VariLCTest  OnPort  BeforeGet  ";

	    CUtils::cameraLog(log);
		pProp->Set(port_.c_str());
	}
	else if (pAct == MM::AfterSet)
	{
		string log = "VariLCTest  OnPort  AfterSet  ";

	    CUtils::cameraLog(log);
		if (initialized_)
		{
			pProp->Set(port_.c_str());
			return DEVICE_INVALID_INPUT_PARAM;
		}
		pProp->Get(port_);
	}

	return DEVICE_OK;
}

 int VariLCTest::OnSerialNumber(MM::PropertyBase* pProp, MM::ActionType eAct)
 {

	 string log = "VariLCTest  OnSerialNumber";

	 CUtils::cameraLog(log);
	 if (eAct == MM::BeforeGet)
	 {
		 int ret = sendCmd("V?", serialnum_);
		 if (ret != DEVICE_OK)return DEVICE_SERIAL_COMMAND_FAILED;
		 pProp->Set(serialnum_.c_str());
	 }
	 return DEVICE_OK;
 }

 
 int VariLCTest::OnWavelength(MM::PropertyBase* pProp, MM::ActionType eAct)
 {
	 string log = "VariLCTest  OnWavelength";

	 CUtils::cameraLog(log);
	 if (eAct == MM::BeforeGet)
	 {
		 std::string ans;
		 int ret = sendCmd("W?", ans);
		 if (ret != DEVICE_OK)return DEVICE_SERIAL_COMMAND_FAILED;
		 vector<double> numbers = getNumbersFromMessage(ans, briefModeQ_);
		 if (numbers.size() == 0) { //The device must have returned "W*" meaning that an invalid wavelength was sent
			 SetErrorText(99, "The Varispec device was commanded to tune to an out of range wavelength.");
			 return 99;
		 }
		 pProp->Set(numbers[0]);
	 }
	 else if (eAct == MM::AfterSet)
	 {
		 double wavelength;
		 // read value from property
		 pProp->Get(wavelength);
		 // write wavelength out to device....
		 ostringstream cmd;
		 cmd.precision(5);
		 cmd << "W " << wavelength;
		 int ret = sendCmd(cmd.str().c_str());
		 if (ret != DEVICE_OK)
			 return DEVICE_SERIAL_COMMAND_FAILED;
		 wavelength_ = wavelength;
	 }
	 return DEVICE_OK;
 }

 int VariLCTest::OnRetardance(MM::PropertyBase* pProp, MM::ActionType eAct, long index)
{
	 string log = "VariLCTest  OnRetardance";

	 CUtils::cameraLog(log);
	if (eAct == MM::BeforeGet)
	{
		std::string ans;
		int ret = sendCmd("L?", ans);
		if (ret != DEVICE_OK)return DEVICE_SERIAL_COMMAND_FAILED;
		vector<double> numbers = getNumbersFromMessage(ans, briefModeQ_);
		if (index < (int)numbers.size()) {
			retardance_[index] = numbers[index];
			pProp->Set(retardance_[index]);

			ostringstream s;
			s << "Retardance LC-" << char(65 + index) << " [in nm.]";
			std::string s2 = DoubleToString(retardance_[index] * wavelength_);
			SetProperty(s.str().c_str(), s2.c_str());

		}
		else {
			return DEVICE_INVALID_PROPERTY_VALUE;
		}
	}
	else if (eAct == MM::AfterSet)
	{
		double retardance;

		// read value from property
		pProp->Get(retardance);
		// write retardance out to device....
		ostringstream cmd;
		cmd.precision(4);
		cmd << "L";
		for (int i = 0; i < numTotalLCs_; i++) {
			if (i == index) {
				cmd << " " << retardance;
			}
			else {
				cmd << " " << retardance_[i];
			}
		}
		int ret = sendCmd(cmd.str());
		if (ret != DEVICE_OK)
			return DEVICE_SERIAL_COMMAND_FAILED;
		retardance_[index] = retardance;

		changedTime_ = GetCurrentMMTime();
	}
	return DEVICE_OK;
}

 int VariLCTest::OnAbsRetardance(MM::PropertyBase* pProp, MM::ActionType eAct, long index)
{	
    string log = "VariLCTest  OnAbsRetardance";

	CUtils::cameraLog(log);

   if (eAct == MM::BeforeGet)
   {	 
     if (index+1 > 0) {	    
	   pProp->Set(retardance_[index]*wavelength_);
	 } else {
		 return DEVICE_INVALID_PROPERTY_VALUE;
	 }
   }
   else if (eAct == MM::AfterSet)
   {
	 
   }
   return DEVICE_OK;
}

int VariLCTest::OnPalEl(MM::PropertyBase* pProp, MM::ActionType eAct, long index)
{
	 string log = "VariLCTest  OnPalEl";

	 CUtils::cameraLog(log);


	 if (eAct == MM::BeforeGet)
	 {
		 std::string ans;
		 int ret = sendCmd("D?", ans);
		 if (ret != DEVICE_OK)return DEVICE_SERIAL_COMMAND_FAILED;
		 vector<double> numbers = getNumbersFromMessage(ans, briefModeQ_);
		 int elemNr = (int)numbers[0];
		 if (elemNr == 0) {
			 for (int i = 0; i < numPalEls_ + 1; i++) {
				 palEl_[i] = "";
			 }
		 }
		 else {
			 for (int i = 1; i <= elemNr; i++) {  //i is one based
				 ret = GetSerialAnswer(port_.c_str(), "\r", ans);
				 if (ret != DEVICE_OK)return DEVICE_SERIAL_COMMAND_FAILED;
				 if ((i - 1) == index) {
					 numbers = getNumbersFromMessage(ans, briefModeQ_);
					 ostringstream palElStr;
					 palElStr.precision(4);
					 palElStr << "  " << (numbers[0]);
					 for (long i = 0; i < numActiveLCs_; ++i) {
						 palElStr << "  " << (numbers[i + 1]);
					 }
					 palEl_[index] = palElStr.str();
				 }
			 }
		 }
		 pProp->Set(palEl_[index].c_str());
	 }
   else if (eAct == MM::AfterSet)
   {
	   long setPalEl = 0;
	   changedTime_ = GetCurrentMMTime();  //enter in each function that sets the LCs
	   ostringstream cmd;
	   cmd.precision(0);
	   pProp->Get(setPalEl);
	   if (setPalEl == 0) {
		   cmd << "D " << index;
		   int ret = sendCmd(cmd.str());
		   if (ret != DEVICE_OK)return DEVICE_SERIAL_COMMAND_FAILED;

	   }
	   if (setPalEl == 1) {
		   cmd << "P " << index;
		   int ret = sendCmd(cmd.str());
		   if (ret != DEVICE_OK)return DEVICE_SERIAL_COMMAND_FAILED;
	   }
	 }
   return DEVICE_OK;
}

int VariLCTest::OnSendToVariLC(MM::PropertyBase* pProp, MM::ActionType eAct)
{

     string log = "VariLCTest  OnSendToVariLC";

	 CUtils::cameraLog(log);
	 if (eAct == MM::AfterSet) {
		 // read value from property
		 pProp->Get(sendToVariLC_);
		 return sendCmd(sendToVariLC_, getFromVariLC_);
	 }
	 return DEVICE_OK;
 }

int VariLCTest::OnGetFromVariLC(MM::PropertyBase* pProp, MM::ActionType eAct)
{

   string log = "VariLCTest  OnGetFromVariLC";

   CUtils::cameraLog(log);
   if (eAct == MM::BeforeGet)
   {
	//   GetSerialAnswer (port_.c_str(), "\r", getFromVariLC_);
      pProp->Set(getFromVariLC_.c_str());
   }
   return DEVICE_OK;
}

 std::vector<double> VariLCTest::getNumbersFromMessage(std::string variLCmessage, bool briefMode) {
   std::istringstream variStream(variLCmessage);
   std::string prefix;
   double val;
   std::vector<double> values;

   string log = "VariLCTest  getNumbersFromMessage";

   CUtils::cameraLog(log);

   if (!briefMode){
      variStream >> prefix;
	}
    for (;;) {
       variStream >> val;
       if (! variStream.fail()) {
          values.push_back(val);
       } else {
           break;
       }
	 }

	return values;
}

int VariLCTest::sendCmd(std::string cmd, std::string& out) {
	
	string log = "VariLCTest  sendCmd 2 param  "  + cmd  + "   port_ =  " + port_;
	if( port_ == "TCP/IP serial port adapter (1)")
	{
	    port_ = "USB2.0";
	}


    CUtils::cameraLog(log);
	sendCmd(cmd);
	GetSerialAnswer(port_.c_str(), "\r", out); //Try returning any extra response from the device.
	return DEVICE_OK;
}


int VariLCTest::sendCmd(std::string cmd) {
	
	string log = "VariLCTest  sendCmd 1 param  "  + cmd +  "     port_  =   " + port_;

    CUtils::cameraLog(log);
	
	int ret = SendSerialCommand(port_.c_str(), cmd.c_str(), "\r");
	if (ret != DEVICE_OK) {
		return DEVICE_SERIAL_COMMAND_FAILED;
	}
	std::string response;
	GetSerialAnswer(port_.c_str(), "\r", response);	//Read back the response and make sure it matches what we sent. If not there is an issue with communication.
	if (response != cmd) {
		SetErrorText(99, "The VariLCTest did not respond.");
		return 99;
	}
	return DEVICE_OK;
}

int VariLCTest::OnBriefMode(MM::PropertyBase* pProp, MM::ActionType eAct)
{

	string log = "VariLCTest  OnBriefMode ";

    CUtils::cameraLog(log);
	if (eAct == MM::BeforeGet) {
		std::string ans;
		int ret = sendCmd("B?", ans);
		if (ret != DEVICE_OK)return DEVICE_SERIAL_COMMAND_FAILED;
		if (ans == "1") {
			briefModeQ_ = true;
		}
		else {
			briefModeQ_ = false;
		}
		if (briefModeQ_) {
			pProp->Set(" 1");
		}
		else {
			pProp->Set(" 0");
		}
	}
	else if (eAct == MM::AfterSet)
	{

	}
	return DEVICE_OK;
}

int VariLCTest::OnNumTotalLCs(MM::PropertyBase* pProp, MM::ActionType eAct)
 {

	 string log = "VariLCTest  OnNumTotalLCs ";

     CUtils::cameraLog(log);
	 if (eAct == MM::BeforeGet)
	 {
		 pProp->Set(numTotalLCs_);
	 }
	 else if (eAct == MM::AfterSet)
	 {
		 pProp->Get(numTotalLCs_);
	 }
	 return DEVICE_OK;
 }


int VariLCTest::OnNumActiveLCs(MM::PropertyBase* pProp, MM::ActionType eAct)
 {
	 string log = "VariLCTest  OnNumActiveLCs ";

     CUtils::cameraLog(log);
	 if (eAct == MM::BeforeGet)
	 {
		 pProp->Set(numActiveLCs_);
	 }
	 else if (eAct == MM::AfterSet)
	 {
		 pProp->Get(numActiveLCs_);
	 }
	 return DEVICE_OK;
 }


