...
MODULE_API MM::Device* CreateDevice(const char* pszDeviceName)
{
  ...
  /*
  CPCO_Camera1 *pcam = new CPCO_Camera1();
  return pcam;
  */
...
{
  PCO_CamList.remove((CPCO_Camera1*) pDevice);
  delete pDevice;
}


CPCO_Camera1::CPCO_Camera1():
CCameraBase<CPCO_Camera1>(),
...
pictime_(0.0)
{
   ...
   CPropertyAction* pAct = new CPropertyAction(this, &CPCO_Camera1::OnDemoMode);
...
void CPCO_Camera1::GetName(char* name) const
{
...
// Camera type
int CPCO_Camera1::OnCameraType(MM::PropertyBase* pProp, MM::ActionType eAct)
{
...
int CPCO_Camera1::OnDemoMode(MM::PropertyBase* pProp, MM::ActionType eAct)
{
  if(eAct == MM::BeforeGet)
  {
    if(m_bDemoMode)
...
int CPCO_Camera1::ResizeImageBuffer()
{
  // get image size
...
int CPCO_Camera1::SetupCamera(bool bStopRecording, bool bSizeChanged)
{
  unsigned int uiMode;
  int nErr = 0;
  int istopresult;
...
int CPCO_Camera1::SetROI(unsigned uX, unsigned uY, unsigned uXSize, unsigned uYSize)
{
  int nErr = 0;
...
int CPCO_Camera1::GetROI(unsigned& uX, unsigned& uY, unsigned& uXSize, unsigned& uYSize)
{
  int nErr = 0;
  if(m_bDemoMode)
  {
...
void CPCO_Camera1::SetExposure(double dExp)
{
  SetProperty(MM::g_Keyword_Exposure, CDeviceUtils::ConvertToString(dExp));
}

int CPCO_Camera1::GetBinning() const
{
  return m_nHBin;
}

int CPCO_Camera1::SetBinning(int binSize)
{
  ostringstream os;
  os << binSize;
...
unsigned CPCO_Camera1::GetBitDepth() const
{
  if(img_.Depth() == 1)
    return 8;
...
unsigned  CPCO_Camera1::GetImageBytesPerPixel() const
{
...
const unsigned char* CPCO_Camera1::GetBuffer(int ibufnum)
{
	if(ibufnum < 0)
        return NULL;
...
const unsigned char* CPCO_Camera1::GetImageBuffer()
{
...
///////////////////////////////////////////////////////////////////////////////
// Function name   : CPCO_Camera1::Initialize
// Description     : Initialize the camera
// Return type     : bool 

int CPCO_Camera1::Initialize()
{
  ...
  pAct = new CPropertyAction(this, &CPCO_Camera1::OnCameraType);
